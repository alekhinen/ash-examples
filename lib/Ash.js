...
var Ash = (function( window, undefined ) {
  ...
  Ash.module('CollectionView', function() {
    ...
    return Ash.View.subclass('CollectionView', {
      ...
      construct: function( opts ) {
        ...
        this.on(this.collection, 'remove', this.removeView.bind(this));
        this.init();
        return this;
      },
      ...
      appendCollection: function() {
        ...
        this._sort();
        _each.call(this._filter(), function( view ) {
          view.el.appendTo(frag);
        }, this);
        target.append(frag);
      },
      ...
      filter: function( key, val ) {
          // custom filter
        var fn = _type.function(key) ? key :
          // no filter
          key === false ? function() {return true;} :
          // key/val matching
          function( view ) {
            return view.model.get(key) === val;
          };
        this._filter = function() {
          return Ash.array.filter(this.views, fn);
        };
        this.appendCollection();
      },
      ...
      sort: function( key, descending ) {
        // custom sort
        var fn = _type.function(key) ? key :
          // no filter
          key === false ? function() {return 0;} :
          // by key
          function( a, b ) {
            a = a.model.get(key);
            b = b.model.get(key);
            return descending === true ? ( b < a ? -1 : b > a ? 1 : 0 ) :
              ( a < b ? -1 : a > b ? 1 : 0 );
          };
        this._sort = function() {
          this.views.sort(fn);
  ...
  Ash.module('Map', function() {
    ...
    return Ash.subclass('Map', Ash.event, {
      ...
      toJSON: function() {
        return _extend.call({}, this._internal);
      }
    });
